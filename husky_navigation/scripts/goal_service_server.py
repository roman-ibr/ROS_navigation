...
# Import the types generated for the service
from navigation_exam.srv import SendPosition, SendPositionResponse

import tf
import yaml

class SendGoalPoseServer():
    def __init__(self):
        ...
        self.s = rospy.Service('send_pose_service', SendPosition, self.__handle_send_goal_label) 
    ...
    def __handle_send_goal_label(self,req):
        resp = SendPositionResponse(True, "OK")
        # Look for label in spots file
        pose = self.__get_pose_from_label(req.label)
        if pose == -1:
            resp = SendPositionResponse(False, "No label found")
        else:
            # Move the base throught the action service move_base
            ret = self.__move_base_client(pose)
